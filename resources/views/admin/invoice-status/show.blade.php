                ...
                <div class="card-body">
                    <table class="table">
                        ...
                        <tr>
                            ...
                            <td>
                                @if ($invoiceStatus->is_active)
                                    <span class="badge bg-success">Active</span>
                                @else
                                    <span class="badge bg-danger">Inactive</span>
                                @endif
                            </td>
                            <td>{{ $invoiceStatus->created_at->format('l, F j, Y') }}</td>
                            <td>{{ $invoiceStatus->updated_at->format('l, F j, Y') }}</td>
                        </tr>
                    </table>
                </div>
            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.show-audit-modal').click(function(e) {
                e.preventDefault();
                const invoiceStatusId = $(this).data('audit-id');
                // Fetch details via AJAX
                $.ajax({
                    url: `{{ route('admin.invoice-status.audit', ':id') }}`.replace(':id',
                        invoiceStatusId),
                    type: 'GET',
            ...
            $('.delete-audit-log').click(function(e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: `{{ route('admin.invoice-status.audit.delete', ':id') }}`
                                .replace(
                                    ':id', invoiceStatusId),
                            type: 'GET',
                            success: function(data) {
