...
@section('content')
    @include('partials.page-header', [
        'title' => 'View Ticket Priority',
        'breadcrumbs' => [
            ['text' => 'Dashboard', 'link' => route('admin.dashboard')],
            ['text' => 'Ticket Priority', 'link' => route('admin.ticket-priority.index')],
            ['text' => 'View', 'link' => null],
        ],
    ])
            ...
            <div class="card">
                <div class="card-body">
                    <table class="table">
                        ...
                        <tr>
                            <td>Created At</td>
                            <td>{{ $ticketPriority->created_at->format('l, F j, Y') }}</td>
                        </tr>
                        <tr>
                            <td>Updated At</td>
                            <td>{{ $ticketPriority->updated_at->format('l, F j, Y') }}</td>
                        </tr>
                    </table>
                </div>
            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.show-audit-modal').click(function(e) {
                e.preventDefault();
                const ticketPriorityId = $(this).data('audit-id');
                // Fetch details via AJAX
                $.ajax({
                    url: `{{ route('admin.ticket-priority.audit', ':id') }}`.replace(':id',
                        ticketPriorityId),
                    type: 'GET',
            ...
            $('.delete-audit-log').click(function(e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: `{{ route('admin.ticket-priority.audit.delete', ':id') }}`
                                .replace(
                                    ':id', ticketPriorityId),
                            type: 'GET',
                            success: function(data) {
