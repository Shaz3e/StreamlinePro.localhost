...
@section('content')
    ...
    {{-- Create Form --}}
    <form action="{{ route('admin.knowledgebase.articles.store') }}" method="POST" class="needs-validation" novalidate
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-12 mb-3">
                                <div class="form-group">
                                    <input type="text" name="slug" id="slug" class="form-control form-control-sm"
                                        placeholder="article-slug-auto-generated" value="{{ old('slug') }}" required>
                                </div>
                                @error('slug')
                                    <div><span class="text-danger">{{ $message }}</span></div>
                                @enderror
                            ...
                            <div class="col-md-12 mb-3">
                                <div class="form-group">
                                    <textarea name="content" class="form-control textEditor">{!! old('content') !!}</textarea>
                                </div>
                                @error('slug')
                                    <div><span class="text-danger">{{ $message }}</span></div>
                                @enderror
                                    ...
                                    <select id="author_id" name="author_id" class="form-control select2" required>
                                        ...
                                        @foreach ($authors as $author)
                                            <option value="{{ $author->id }}"
                                                {{ old('author_id', auth()->guard('admin')->user()->id) == $author->id ? 'selected' : '' }}>
                                                {{ $author->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-12 mb-3">
                                <div class="form-group">
                                    <select id="is_published" name="is_published" class="form-control" required>
                                        <option value="1" {{ old('is_published') == 1 ? 'selected' : '' }}>Publish
                                        </option>
                            ...
                            <div class="col-12 mb-3">
                                <div class="form-group">
                                    <input type="file" name="featured_image" id="featured_image" class="form-control"
                                        id="customFile">
                                    <small class="d-block text-muted">Only JPG, JPEG, PNG, GIF, SVG with max 2MB file size allowed.</small>
                                </div>
