                ...
                <div class="card-body">
                    <table class="table">
                        ...
                        <tr>
                            ...
                            <td>
                                @if ($invoiceLabel->is_active)
                                    <span class="badge bg-success">Active</span>
                                @else
                                    <span class="badge bg-danger">Inactive</span>
                                @endif
                            </td>
                            <td>{{ $invoiceLabel->created_at->format('l, F j, Y') }}</td>
                            <td>{{ $invoiceLabel->updated_at->format('l, F j, Y') }}</td>
                        </tr>
                    </table>
                </div>
                {{-- /.card-body --}}
        ...
        <script>
            $(document).ready(function() {
                ...
                $('.show-audit-modal').click(function(e) {
                    e.preventDefault();
                    const invoiceLabelId = $(this).data('audit-id');
                    // Fetch details via AJAX
                    $.ajax({
                        url: `{{ route('admin.invoice-labels.audit', ':id') }}`.replace(':id',
                            invoiceLabelId),
                        type: 'GET',
                ...
                $('.delete-audit-log').click(function(e) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            $.ajax({
                                url: `{{ route('admin.invoice-labels.audit.delete', ':id') }}`
                                    .replace(
                                        ':id', invoiceLabelId),
                                type: 'GET',
                                success: function(data) {
