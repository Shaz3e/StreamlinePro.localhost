            ...
            <div class="card">
                <div class="card-body">
                    <table class="table">
                        ...
                        <tr>
                            <td>{{ $todoLabel->name }}</td>
                            <td>{{ $todoLabel->todos->count() }}</td>
                            <td>
                                @if ($todoLabel->is_active)
                                    <span class="badge bg-success">Active</span>
                                @else
                                    <span class="badge bg-danger">Inactive</span>
                                @endif
                            </td>
                            <td>{{ $todoLabel->created_at->format('l, F j, Y') }}</td>
                        </tr>
                    </table>
                </div>
                {{-- /.card-body --}}
                <div class="card-footer">
                    <a href="{{ route('admin.todo-labels.edit', $todoLabel->id) }}" class="btn btn-sm btn-outline-success">
        ...
        <script>
            $(document).ready(function() {
                ...
                $('.show-audit-modal').click(function(e) {
                    e.preventDefault();
                    const todoLabelId = $(this).data('audit-id');
                    // Fetch details via AJAX
                    $.ajax({
                        url: `{{ route('admin.todo-labels.audit', ':id') }}`.replace(':id',
                            todoLabelId),
                        type: 'GET',
                ...
                $('.delete-audit-log').click(function(e) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            $.ajax({
                                url: `{{ route('admin.todo-labels.audit.delete', ':id') }}`
                                    .replace(
                                        ':id', todoLabelId),
                                type: 'GET',
                                success: function(data) {
