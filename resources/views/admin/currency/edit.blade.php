...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <form action="{{ route('admin.settings.currencies.update', $currency->id) }}" method="POST"
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="is_active" required>
                                        <option value="1"
                                            {{ old('is_active', $currency->is_active) == 1 ? 'selected' : '' }}>Show
                                        </option>
                                        <option value="0"
                                            {{ old('is_active', $currency->is_active) == 0 ? 'selected' : '' }}>Hide
                                        </option>
                                    </select>
                                    @error('is_active')
                                        <span class="text-danger">{{ $message }}</span>
