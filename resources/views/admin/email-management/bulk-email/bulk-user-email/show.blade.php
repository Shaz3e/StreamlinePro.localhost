...
@section('content')
    ...
    <div class="row mb-3">
        <div class="col-md-3">
            ...
            <div class="card-text">
                @if (!is_null($bulkEmailUser->user_id))
                    @if ($bulkEmailUser->is_sent_all_users)
                        <span class="badge bg-success">Sent to All Users</span>
                    @else
                        <span class="badge bg-info">{{ count($bulkEmailUser->user_id) }} </span>
                        User(s)
                    @endif
                @endif
                @if (!is_null($bulkEmailUser->admin_id))
                    @if ($bulkEmailUser->is_sent_all_admins)
                        <span class="badge bg-success">Sent to All Staff</span>
                    @else
                        <span class="badge bg-info">{{ count($bulkEmailUser->admin_id) }} </span>
                        Staff Members
                    @endif
                @endif
            </div>
        ...
        <div class="col-md-3">
            ...
            <div class="card-text">
                {!! $bulkEmailUser->getPublishStatusBadge() !!}
            </div>
        </div>
        {{-- /.col --}}
        <div class="col-md-3">
            ...
            <div class="card-text">
                {!! $bulkEmailUser->getSentStatusBadge() !!}
            </div>
        </div>
        {{-- /.col --}}
        <div class="col-md-3">
            ...
            <div class="card-text">
                {{ $bulkEmailUser->send_date->format('l, jS M Y h:i A') }}
            </div>
        </div>
        {{-- /.col left --}}
    </div>
                ...
                <div class="card-footer">
                    <div class="row">
                        <div class="col-6">
                            ...
                            {{ $bulkEmailUser->created_at->format('l, jS M Y h:i A') }}
                        </div>
                        {{-- /.col left --}}
                        <div class="col-6 text-end">
                            <small class="d-block">Updated At</small>
                            {{ $bulkEmailUser->updated_at->format('l, jS M Y h:i A') }}
                        </div>
                        {{-- /.col right --}}
                    </div>
                    {{-- /.row --}}
    ...
    @can('bulk-email.update')
        <div class="row">
            <div class="col-12">
                ...
                @else
                    <a href="{{ route('admin.email-management.bulk-emails.edit', $bulkEmailUser->id) }}">
