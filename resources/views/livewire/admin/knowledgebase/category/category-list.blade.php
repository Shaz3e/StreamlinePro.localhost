<div>
    <div class="row mb-3">
        ...
        <div class="col-md-2 col-sm-12 mb-2">
            <div class="d-grid">
                <a href="{{ route('admin.knowledgebase.categories.create') }}"
                            ...
                            <tbody>
                                ...
                                @foreach ($categories as $category)
                                    <tr wire:key="{{ $category->id }}">
                                        ...
                                        @if (!$showDeleted)
                                            <td>
                                                <input type="checkbox" wire:change="toggleStatus({{ $category->id }})"
                                                    id="is_active_{{ $category->id }}" switch="bool"
                                                    {{ $category->is_active ? 'checked' : '' }} />
                                                <label for="is_active_{{ $category->id }}" data-on-label="Yes"
                                                    data-off-label="No"></label>
                                            </td>
                                        @endif
                                        <td class="text-right">
                                            @if ($showDeleted)
                                                @can('knowledgebase-category.restore')
                                                ...
                                                @endcan
                                                @can('knowledgebase-category.force.delete')
                                                    <button wire:click="confirmForceDelete({{ $category->id }})"
                                                ...
                                                @endcan
                                            @else
                                                @can('knowledgebase-category.read')
                                                    <a href="{{ route('admin.knowledgebase.categories.show', $category->id) }}"
                                                        class="btn btn-sm btn-outline-info">
                                                        <i class="ri-eye-line"></i>
                                                    </a>
                                                @endcan
                                                @can('knowledgebase-category.update')
                                                    <a href="{{ route('admin.knowledgebase.categories.edit', $category->id) }}"
                                                        class="btn btn-sm btn-outline-success">
                                                        <i class="ri-pencil-line"></i>
                                                    </a>
                                                @endcan
                                                @can('knowledgebase-category.delete')
