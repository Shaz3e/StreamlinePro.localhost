<div>
    <div class="row mb-3">
        ...
        <div class="col-md-2 col-sm-12 mb-2">
            <div class="d-grid">
                <a href="{{ route('admin.settings.currencies.create') }}" class="btn btn-success btn-sm waves-effect waves-light">
                        ...
                        <table id="data" class="table table-striped table-hover">
                            ...
                            <tbody>
                                @foreach ($currencies as $currency)
                                    <tr wire:key="{{ $currency->id }}">
                                        <td>{{ $currency->name }}</td>
                                        <td>{{ $currency->symbol }}</td>
                                        @if (!$showDeleted)
                                            <td>
                                                <input type="checkbox" wire:change="toggleStatus({{ $currency->id }})"
                                                    id="is_active_{{ $currency->id }}" switch="bool"
                                                    {{ $currency->is_active ? 'checked' : '' }} />
                                                <label for="is_active_{{ $currency->id }}" data-on-label="Yes"
                                        ...
                                        <td class="text-right">
                                            @if ($showDeleted)
                                                @can('currency.restore')
                                                    <button wire:click="confirmRestore({{ $currency->id }})"
                                                        class="btn btn-sm btn-outline-info" data-toggle="modal"
                                                        data-target="#deleteModal">
                                                        <i class="ri-arrow-go-back-line"></i>
                                                    </button>
                                                @endcan
                                                @can('currency.force.delete')
                                                    <button wire:click="confirmForceDelete({{ $currency->id }})"
                                            ...
                                            @else
                                                @can('currency.update')
                                                    <a href="{{ route('admin.settings.currencies.edit', $currency->id) }}"
