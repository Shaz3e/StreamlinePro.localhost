<div>
    <div class="row mb-3">
        ...
        <div class="col-md-2 col-sm-12 mb-2">
            <div class="d-grid">
                <a href="{{ route('admin.ticket-priority.create') }}"
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <table id="data" class="table table-striped table-hover">
                        ...
                        <tbody>
                            @php
                                $totalRecords = $ticketPriorityList->total();
                                $currentPage = $ticketPriorityList->currentPage();
                                $perPage = $ticketPriorityList->perPage();
                                $id = $totalRecords - ($currentPage - 1) * $perPage;
                            @endphp
                            @foreach ($ticketPriorityList as $ticketPriority)
                                <tr wire:key="{{ $ticketPriority->id }}">
                                    <td>{{ $id-- }}</td>
                                    <td>
                                        <span class="badge"
                                            style="background-color:{{ $ticketPriority->bg_color }}; color:{{ $ticketPriority->text_color }}">
                                            {{ $ticketPriority->name }}
                                        </span>
                                    </td>
                                    @if (!$showDeleted)
                                        <td>
                                            <input type="checkbox" wire:change="toggleStatus({{ $ticketPriority->id }})"
                                                id="is_active_{{ $ticketPriority->id }}" switch="bool"
                                                {{ $ticketPriority->is_active ? 'checked' : '' }} />
                                            <label for="is_active_{{ $ticketPriority->id }}" data-on-label="Yes"
                                                data-off-label="No"></label>
                                        </td>
                                    @endif
                                    <td class="text-right">
                                        @if ($showDeleted)
                                            @can('ticket-priority.restore')
                                                <button wire:click="confirmRestore({{ $ticketPriority->id }})"
                                                    class="btn btn-sm btn-outline-info" data-toggle="modal"
                                                    data-target="#deleteModal">
                                                    <i class="ri-arrow-go-back-line"></i>
                                                </button>
                                            @endcan
                                            @can('ticket-priority.force.delete')
                                                <button wire:click="confirmForceDelete({{ $ticketPriority->id }})"
                                            ...
                                            @endcan
                                        @else
                                            @can('ticket-priority.read')
                                                <a href="{{ route('admin.ticket-priority.show', $ticketPriority->id) }}"
                                                    class="btn btn-sm btn-outline-info">
                                                    <i class="ri-eye-line"></i>
                                                </a>
                                            @endcan
                                            @can('ticket-priority.update')
                                            ...
                                            @can('ticket-priority.delete')
                                                <button wire:click="confirmDelete({{ $ticketPriority->id }})"
                                                    class="btn btn-sm btn-outline-danger" data-toggle="modal"
                                                    data-target="#deleteModal">
